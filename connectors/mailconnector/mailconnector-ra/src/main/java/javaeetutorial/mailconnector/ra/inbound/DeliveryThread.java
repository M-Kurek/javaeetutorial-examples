 ...
 */
package javaeetutorial.mailconnector.ra.inbound;
...
public class DeliveryThread implements Work, WorkContextProvider {
    ...
    static final Logger logger = 
            Logger.getLogger("javaeetutorial.mailconnector.ra.inbound");
    ...
    @Override
    public void release() {
        logger.info("[Inbound:DeliveryThread] Worker Manager called release for deliveryThread ");
    }
    ...
    @Override
    public void run() {
        logger.info("[Inbound:DeliveryThread] WorkManager started delivery thread ");

        try {
            endpointConsumer.deliverMessage(msg);
        } catch (Exception te) {
            logger.log(Level.SEVERE, "Inbound:DeliveryThread::run got an exception: {0}", 
                    te.getMessage());
        }
    ...
    private void initializeWorkContexts(Message msg) {
        try {
            logger.info("[Inbound:DeliveryThread] Calling initializeWorkContexts");
            ...
            if (recipients != null && recipients.length > 0) {
                ...
                if (recipientId.indexOf("@") > 0) {
                    recipientId = recipientId.substring(0, recipientId.indexOf("@"));
                }

                //Assuming that the password is same as username
                MySecurityContext mysc = new MySecurityContext(recipientId, recipientId, recipientId);
