...
public class JavaMailConnectionImpl implements JavaMailConnection {
    ...
    public ConnectionMetaData getMetaData()
            throws ResourceException {
        logger.log(Level.INFO,
                " -- Outbound:JavaMailConnectionImpl:: getMetaData mc={0}", mc);
    ...
    public void close()
            throws ResourceException {
        logger.log(Level.INFO,
                " -- Outbound:JavaMailConnectionImpl:: close mc={0}", mc);
        if (mc == null) {
    ...
    public void associateConnection(ManagedConnectionImpl newMc)
            throws ResourceException {
        logger.info("[Outbound:JavaMailConnectionImpl] Calling associateConnection");
        checkIfValid();
    ...
    void checkIfValid()
            throws ResourceException {
        logger.log(Level.INFO,
                " -- Outbound:JavaMailConnectionImpl::checkIfValid mc={0}", mc);
        if (mc == null) {
    ...
    void invalidate() {
        logger.log(Level.INFO,
                " -- Outbound:JavaMailConnectionImpl::invalidate mc={0}", mc);
        mc = null;
    }
    ...
    public String[] getNewMessageHeaders()
            throws ResourceException {
        logger.info("[Outbound:JavaMailConnectionImpl] Calling getNewMessageHeaders");
        checkIfValid();
        try {
