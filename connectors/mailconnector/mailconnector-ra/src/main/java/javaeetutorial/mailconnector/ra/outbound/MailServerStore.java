...
public class MailServerStore {
    ...
    private void connectStore()
        ...
        try {
            ...
            logger.log(Level.SEVERE, "[Outbound:MailServerStore] Caught an exception when obtaining a "
                    + "JavaMail Session: {0}", te.toString());
    ...
    public Folder getFolder(String folderName)
        ...
        if ((folder == null) || (!folder.exists())) {
            Exception e = new Exception("[Outbound:MailServerStore] Folder " + folderName
    ...
    public Message[] getNewMessages(Folder folder)
        ...
        if ((folder == null) || (!folder.exists())) {
            Exception e = new Exception("[Outbound:MailServerStore] Folder " + folder
                    + " does not exist or is not found");
            throw e;
        }
        ...
        try {
            ...
            if (newMsgs > 0) {
                ...
                Message msgs[] =
                        // changed because mock-javamail seems to count from
                        // 0, not 1
            ...
            logger.severe("[Outbound:MailServerStore] Exception obtaining messages from mail server");
        }
        return null;
    }
        ...
        try {
            ...
            if (newMsgs > 0) {
                ...
                logger.log(Level.FINE, 
                        "[Outbound:MailServerStore] messages length: {0}", msgs.length);
                logger.log(Level.FINE, 
                        "[Outbound:MailServerStore] headers length: {0}", headers.length);
            ...
            logger.severe("[Outbound:MailServerStore] Exception obtaining message headers from mail server:");
        }
        return null;
    }
